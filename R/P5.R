...
# Sirve para normalizar la distribución de los datos, siendo así mucho más visible.
data2 = log2(data)
...
# ¿Qué es un boxplot?
# Un boxplot representa la distribución de datos por cuartiles
# La línea central indica la median, la caja el rango interquartílico q1-q3,
# los bigotes muestran los valores más extremos pero dentro del rango 1,5, y los puntos fuera de 
# los bigotes muestran valores fuera de rango
...
# Sí porque los KOs están juntos y a su vez separados de los wildtype, también agrupados entre sí
...
# Se generan dos matrices numéricas
wt <- data[,1:3]
ko <- data[,4:6]
class(wt)
...
# Porque queremos usar datos normalizados, sin transformar, evaluando diferencias absolutas
# ¿Cuántas valores tiene cada muestra?
# 12488 valores distribuidos en dos grupos, con tres réplicas cada uno.
pvalue = NULL 
tstat = NULL 
...
# Para poder diferenciar mejor valores significativos de tamaños pequeños y apreciarlos visualmente
...
# Porque un gen ko que está sobreexpresado es más grande que el wt, lo que dará una diferencia negativa en la resta
...
# rowv=fila, colv=columna, ambos representan los clusters
# cexcol para ajustar los tamaños de los clusters
# labRow=FALSE sirve para ocultar los nombres de todos los genes
